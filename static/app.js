...
function showDone(g) {
  ...
  const coopStats = g.kind === "coop" ? `
    <div class="winstats">
      <div class="ws-row"><span class="ws-dot" style="background:var(--p0)"></span>${cap(ROSTER[0])}</span><b>${p0N}</b></div>
      <div class="ws-row"><span class="ws-dot" style="background:var(--p1-soft)"></span>${cap(ROSTER[1])}</span><b>${p1N}</b></div>
    </div>` : "";
  const mk = g.mistakes || {};
  wrap.innerHTML = `<div class="done">
    ...
    ${coopStats}
    <div class="sub">${g.kind === "coop" ? `${cap(ROSTER[0])} & ${cap(ROSTER[1])} — another one down together` : "A clean solo solve"}</div>
